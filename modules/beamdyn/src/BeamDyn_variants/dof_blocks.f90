   ...
   IF (fact) THEN  
       DO j=1,p%nodes_per_elem
               DO i=1,p%nodes_per_elem
                       ...
                       DO idx_qp = 1,p%nqp
                           m%elk(:,i,:,j) = m%elk(:,i,:,j) + (m%qp%Qe(:,:,idx_qp,nelem) +  m%qp%Ki(:,:,idx_qp,nelem))*p%QPtw_Shp_Shp_Jac(idx_qp,i,j,nelem) &
                                                           +  m%qp%Pe(  :,:,idx_qp,nelem)*p%QPtw_Shp_ShpDer(idx_qp,i,j) &
                                                           +  m%qp%Oe(  :,:,idx_qp,nelem)*p%QPtw_Shp_ShpDer(idx_qp,j,i) &
                                                           +  m%qp%Stif(:,:,idx_qp,nelem)*p%QPtw_ShpDer_ShpDer_Jac(idx_qp,i,j,nelem)
                  
                           m%elm(:,i,:,j) = m%elm(:,i,:,j) + m%qp%Mi(:,:,idx_qp,nelem)*p%QPtw_Shp_Shp_Jac(idx_qp,i,j,nelem)

                           m%elg(:,i,:,j) = m%elg(:,i,:,j) + m%qp%Gi(:,:,idx_qp,nelem)*p%QPtw_Shp_Shp_Jac(idx_qp,i,j,nelem)

                           IF (p%damp_flag .NE. 0) THEN
                               m%elk(:,i,:,j) = m%elk(:,i,:,j) + m%qp%Qd(:,:,idx_qp,nelem)*p%QPtw_Shp_Shp_Jac(idx_qp,i,j,nelem) &
                                                               + m%qp%Pd(:,:,idx_qp,nelem)*p%QPtw_Shp_ShpDer(idx_qp,i,j) &
                                                               + m%qp%Od(:,:,idx_qp,nelem)*p%QPtw_Shp_ShpDer(idx_qp,j,i)  &
                                                               + m%qp%Sd(:,:,idx_qp,nelem)*p%QPtw_ShpDer_ShpDer_Jac(idx_qp,i,j,nelem)

                               m%elg(:,i,:,j) = m%elg(:,i,:,j) + m%qp%Xd(   :,:,idx_qp,nelem)*p%QPtw_Shp_Shp_Jac(idx_qp,i,j,nelem) &
                                                               + m%qp%Yd(   :,:,idx_qp,nelem)*p%QPtw_Shp_ShpDer(idx_qp,i,j) &
                                                               + m%qp%Gd(   :,:,idx_qp,nelem)*p%QPtw_Shp_ShpDer(idx_qp,j,i) &
                                                               + m%qp%betaC(:,:,idx_qp,nelem)*p%QPtw_ShpDer_ShpDer_Jac(idx_qp,i,j,nelem)
   ...
   DO i=1,p%nodes_per_elem
         ...
         DO idx_qp = 1,p%nqp
            m%elf(:,i) = m%elf(:,i) - m%qp%Fc(:,idx_qp,nelem)*p%QPtw_ShpDer(idx_qp,i) &
                                    - (m%qp%Ftemp(:,idx_qp,nelem) + m%qp%Fi(:,idx_qp,nelem))*p%QPtw_Shp_Jac(idx_qp,i,nelem)
         END DO
